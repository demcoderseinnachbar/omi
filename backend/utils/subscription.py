...
from utils.byok import get_byok_key, get_byok_llm_provider, get_byok_keys, has_byok_keys
...
def _request_has_llm_byok_key() -> bool:
    ...
    keys = get_byok_keys()
    return any(
        get_byok_key(provider) or keys.get(provider) for provider in ('openrouter', 'openai', 'gemini', 'anthropic')
    )
