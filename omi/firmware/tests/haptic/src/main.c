...
static void reset_haptic(void *fixture)
{
	...
	(void) k_work_cancel_delayable_sync(&haptic_off_work,
...
ZTEST(haptic_ownership, test_b_a_stale_handler_leaves_a_newer_pulse_alone)
{
	...
	zassert_true(motor_on(),
		     "a stale handler switched off a pulse that had not ended");
}
...
ZTEST(haptic_ownership, test_e_an_explicit_off_does_not_poison_the_next_pulse)
{
	...
	k_sleep(K_MSEC(100));
	zassert_true(motor_on(),
		     "a leftover from the explicit off ended the next pulse");
...
ZTEST(haptic_ownership, test_the_five_second_cap_still_holds)
{
	...
	zassert_true(haptic_off_at <= capped + k_ms_to_ticks_ceil64(50),
		     "a request beyond the cap was not capped");
